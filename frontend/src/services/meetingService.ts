import api from './api';
import { Meeting, CreateMeetingRequest, MeetingSeries, ProcessingStatus, UploadResponse, ActionItem, AttendeesResponse, MeetingSummary } from '@/types/meeting';
...
export const meetingService = {
  ...
  startProcessing: async (meetingId: string): Promise<Meeting> => {
    const response = await api.post<Meeting>(
      `/api/v1/meetings/${meetingId}/process`,
    );
    return response.data;
  },
  ...
  getMeeting: async (meetingId: string): Promise<Meeting> => {
    ...
    try {
      console.log(
        `[meetingService] Sending request to /api/v1/meetings/${meetingId}`,
      );
      ...
      if (status === 404) {
        ...
        const errorMessage =
          errorData?.message || axiosError.message || "Failed to fetch meeting";
  ...
  getMeetings: async (params?: {
    ...
    const response = await api.get<{ data: Meeting[] }>("/api/v1/meetings", {
      params,
    });
    return response.data;
  },
   ...
   */
  getMeetingSeries: async (recentMeetings: number = 3): Promise<MeetingSeries[]> => {
    const response = await api.get<MeetingSeries[]>(`/api/v1/meeting-series?recentMeetings=${recentMeetings}`);
    return response.data;
  },
   ...
   */
  createMeetingSeries: async (data: { title: string; description?: string }): Promise<MeetingSeries> => {
    const response = await api.post<MeetingSeries>('/api/v1/meeting-series', data);
    return response.data;
  },

  /**
   * Get meetings in a series
   */
  getSeriesMeetings: async (seriesId: string): Promise<MeetingSummary[]> => {
    const response = await api.get<MeetingSummary[]>(`/api/v1/meeting-series/${seriesId}/meetings`);
    return response.data;
  },
   ...
   */
  updateMeetingSeries: async (seriesId: string, data: { title?: string; description?: string }): Promise<MeetingSeries> => {
    const response = await api.put<MeetingSeries>(`/api/v1/meeting-series/${seriesId}`, data);
    return response.data;
  },
   ...
   */
  deleteMeetingSeries: async (seriesId: string): Promise<void> => {
    await api.delete(`/api/v1/meeting-series/${seriesId}`);
  },
  ...
  updateTaskStatus: async (
    ...
    const response = await api.patch<ActionItem>(
      `/api/v1/action-items/${taskId}/status`,
      {
        status,
      },
    );
  ...
  reassignTask: async (
    ...
    const response = await api.patch<ActionItem>(
      `/api/v1/action-items/${taskId}/assign`,
      {
        assignee,
      },
    );
